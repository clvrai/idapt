...
class BaseEnv(gym.Env):
    ...
    def __init__(self, xml_path, background="Simple", **kwargs):
        ...
        self.joint_space = spaces.Dict(
            [("ac", spaces.Box(low=jnt_minimum, high=jnt_maximum, dtype=np.float32),)]
        )
    ...
    def _load_model_from_path(self, xml_path):
        ...
        self.action_space = spaces.Dict(
            [("ac", spaces.Box(low=minimum, high=maximum, dtype=np.float32))]
        )
    ...
    def reset(self):
        ...
        if self.render_mode == "human" and not self._unity:
    ...
    def render(
        self, mode="human", close=False, height=None, width=None, camera_id=None
    ):
    ...
    def has_contact(self, geom1, geom2):
        ...
        geom1_id = self.sim.model.geom_name2id(geom1)
